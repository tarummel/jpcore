...
class KanjiViewsTestCase(TestCase):
    ...
    def test_get_by_id_bad_request(self):
        url = self.helper.getEntryByCharacterUrl(None)
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.BAD_REQUEST)

        url = self.helper.getEntryByCharacterUrl('too long')
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.BAD_REQUEST)

    def test_get_by_char_success(self):
        url = self.helper.getEntryByCharacterUrl(self.kanji1.content)
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.OK)
        ...
        self.assertTrue('jkanji' in data[0])
        self.assertTrue('jreading' in data[0])
        self.assertTrue('jsense' in data[0])

        jk, jr, jse = data[0]['jkanji'], data[0]['jreading'], data[0]['jsense']
        self.assertEqual(len(jk), 2)
        self.assertEqual(len(jr), 2)
        self.assertEqual(len(jse), 2)
    ...
    def test_get_by_char_not_found(self):
        url = self.helper.getEntryByCharacterUrl('')
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.NOT_FOUND)
        ...
        url = self.helper.getEntryByCharacterUrl('人')
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.NOT_FOUND)

    def test_get_by_char_bad_request(self):
        url = self.helper.getEntryByCharacterUrl(None)
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.BAD_REQUEST)
